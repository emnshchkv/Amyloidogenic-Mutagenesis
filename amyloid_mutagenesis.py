...
class AmyloidMutagenesis:
    ...
    def __init__(
        self, sequence: str, regions: List[Tuple[int, int]], output_file: str
    ) -> None:
        """
        ...
        for start, end in self.regions:
            if start < 1 or end > len(self.original_sequence) or start > end:
                raise ValueError(
                    f"Invalid region {start}:{end} for sequence length {len(self.original_sequence)}"
                )
...
def parse_regions(region_string: str) -> List[Tuple[int, int]]:
    ...
    for region in region_string.split(","):
        ...
        try:
            ...
            raise ValueError(
                f"Invalid region format: {region}. Start and end must be integers."
            )
