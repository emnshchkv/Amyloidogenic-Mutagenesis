...
def trim_sequences(input_fasta, output_fasta, start_res, end_res):
    ...
    try:
        with (
            open(input_fasta, "r", encoding="utf-8") as infile,
...
def main():
    ...
    parser.add_argument(
        "-i", "--input", required=True, help="Path to the input FASTA/multi-FASTA file."
    )
    parser.add_argument(
        "-s",
        "--start",
    ...
    if args.start >= args.end or args.start < 1:
        print(
            "Error: Invalid coordinates. Ensure that start < end and start >= 1.",
            file=sys.stderr,
        )
        sys.exit(1)
